    ...
    rpcConsole(0)
{
    setMinimumSize(870,620);
            resize(870,620);
    setWindowTitle(tr("InterCash") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a InterCash address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About InterCash"), this);
    aboutAction->setToolTip(tr("Show information about InterCash"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for InterCash"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("InterCash client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("InterCash client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to InterCash network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid InterCash address or malformed URI parameters."));
    }
