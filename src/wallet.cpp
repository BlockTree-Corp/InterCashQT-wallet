...
// InterCash: total coins staked (non-spendable until maturity)
...
// InterCash: get current stake weight
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
		...
		//InterCash now iterates each utxo inside of CheckStakeKernelHash()
...
// InterCash: check 'spent' consistency between wallet and txindex
// InterCash: fix wallet spent state according to txindex
...
// InterCash: disable transaction (only for coinstake)
